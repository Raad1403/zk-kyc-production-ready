...
> **🚀 DEPLOYED ON BASE SEPOLIA L2 — TESTNET-VALIDATED (TECH-COMPLETE & MAINNET-READY)**
...
- **Tech-complete & Mainnet-ready**: Testnet-validated with measured performance
- **Enterprise Grade**: Nullifier system prevents replay attacks

### **Market Opportunity**
- **Privacy Regulations**: Helps meet key privacy requirements in GDPR/CCPA (no on-chain PII, data minimization)
...
> For full deployment artifacts and gas details, see `deployed-baseSepolia.json`.
...
- **Groth16 Verifier**: Production verifier for ZK proofs
...
### **3. Enterprise Licensing: $10,000-50,000**
- Custom deployment
- Technical support
...
This project is a privacy-preserving KYC layer that integrates with traditional KYC providers:

- Off-chain provider (e.g., Onfido/Veriff) performs document/AML checks and publishes signed Merkle roots.
- `Registry` accepts/rotates valid roots on-chain.
- Users generate a Groth16 proof showing membership in a valid root and a one-time `nullifier` bound to a `signalHash`.
- On-chain, we only verify the root and ensure the `nullifier` is unused, then mark it used—no PII ever touches the chain.

This separation keeps sensitive data off-chain while enabling on-chain gating for DeFi actions.
...
This privacy-preserving KYC layer is **tech-complete & mainnet-ready (testnet-validated)** with:
